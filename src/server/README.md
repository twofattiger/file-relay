...
* **双模式收发**:
  * **方式1 · 分享发送 (`/send` → `/r/{id}`)**: 选择文件即生成一次性链接 / 二维码，发给对方匿名接收，固定 sender↔receiver 两端。
  * **方式2 · 设备互传 (`/room` → `/m/{密码}`)**: 创建房间后多台设备凭同一链接加入，服务端维护设备名册并转发 WebRTC 信令，设备两两之间端到端 mesh 直连，可双向、多次互发文件（每对设备一张卡片）。
* **混合传输架构 (WebRTC P2P + 中继)**: 默认优先尝试建立 **WebRTC 点对点直连**，数据不经过服务器，局域网或良好网络环境下传输速度最大化。若由于复杂 NAT/防火墙导致打洞失败，或用户主动关闭 P2P，系统将平滑、无缝地回退到 Go WebSocket 服务端中继模式（方式2 回退时为每对设备建立一条独立的 `relay` 配对会话兜底）。
